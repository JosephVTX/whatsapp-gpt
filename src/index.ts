...
whatsapp.then(async sock => {
    ...
    sock.ev.on("messages.upsert", async ({ messages }) => {
        ...
        const message: string = messages[0].message?.conversation || messages[0].message?.extendedTextMessage?.text || "";
        ...
        if (message.startsWith("/ia")) {
            ...
            try {
                ...
                await sock.sendMessage(remoteJid, { text: messageGpt }, { quoted: messageInfo });

            }

            catch {

                await sock.sendMessage(remoteJid, { text: "Error en el backend" }, { quoted: messageInfo });
            }
